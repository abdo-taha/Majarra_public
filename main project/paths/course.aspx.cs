    ...
    public partial class course : System.Web.UI.Page
    {
        ...
        string load_courses( string sub_path_id)
        {
            ...
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("SELECT id,name,level,type,sub_path_id,rate, course_link from courses_data where sub_path_id = '" + sub_path_id + "';", con);
                SqlDataReader dr = cmd.ExecuteReader();
                ...
                if (dr.HasRows)
                {
                    while (dr.Read())
                    {
                        ...
                        c_info.course_link = dr[6].ToString();
        ...
        string courses_to_html(course_info c_inf )
        {
            ...
            str.Append(string.Format("			  					 		 <a class=\"card-title\" href=\"{0}\" style=\"font-size:1.5rem; text-decoration:none; color:black;\" target=\"_blank\">{1}</a>\n", c_inf.course_link ,c_inf.course_name));
        ...
        class course_info
        {
           public string course_name="no_name", course_rate="0", added_by="no one", profile_link="#",
                comments_link="#", comments_count="0", course_level="beginner", course_type="video" , course_link="#";
        }
